...
import { formatCurrency, formatPercentage, toSafeNumber } from "../utils/format";
              ...
              <Link to={`/coin/${coin.id}`} key={coin.id} className="block">
                <div className="glass-card p-6 hover:shadow-xl transition-all bg-white dark:bg-dark-200 rounded-2xl relative group">
                  ...
                  <div className="flex justify-between items-end">
                    ...
                    <div className={`flex items-center ${priceChange24h >= 0 ? "text-success" : "text-error"}`}>
                      {priceChange24h >= 0 ?
                        <TrendingUp className="w-4 h-4 mr-1" /> :
                        <TrendingDown className="w-4 h-4 mr-1" />
                      }
                      <span className="font-bold">{formatPercentage(Math.abs(priceChange24h))}</span>
                    </div>
                  </div>
                </div>
              </Link>
